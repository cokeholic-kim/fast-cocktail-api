# FastAPI 마이그레이션 API 실행 가이드

## 프로젝트 구조
- `fastapi-app/app`: API/설정/DB/모델/스키마/서비스 코드
- `fastapi-app/alembic`: DB 마이그레이션 설정
...
3. 환경변수 파일 준비
   ...
   - 로컬 DB 사용 시 `DATABASE_URL`을 `postgresql+asyncpg://postgres:postgres@localhost:5432/cocktail_db`로 설정
